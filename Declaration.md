    ...
    **Generated_page** 类的方法：
    ...
    1. **__init__** 初始化方法，参数：
        ...
        - **proxy** 字典，默认为空字典，结构：{
                ...
                'bypass': '不走代理的地址，每个用逗号分隔，默认为 None'
            }
    ...
    1. **click** 直接点击页面元素，先移动，再点击，参数：
        - **CssSelector_or_JSHandle** __CSS选择器__ 或 __JSHandle 对象__
            //为 __字符串__ 时，认为是CSS选择器，查找并移动到标签并点击
            //为 __JSHandle 对象__ 时，认为是一个JS对象，直接调用JS的 __.click()__ 方法点击
        - **DOM_text** 元素的文本内容，为 __None__ 时直接用CSS查找元素，为__字符串__时表示指定元素的文本内容
    ...
    1. **get_element_coordinate** 获取页面元素坐标，参数：
        - **css_selector** CSS选择器，用于查找页面元素
        - **DOM_text** 元素的文本内容，为 __None__ 时直接用CSS查找元素，为 __字符串__ 时表示指定元素的文本内容
