作者：伊茗(QQ：__2368199809__，邮箱：__2368199809@qq.com__)
...
1. 对于多个需要此文件的项目，可以将此文件放在一个固定文件夹，写一个固定逻辑引入
    ```Python
    import sys
...
1. **Generated_page.py** 核心库，里面定义了一个核心类 **Generated_page**
    ...
    **Generated_page** 类的方法：
    ...
    1. **__init__** 初始化方法，参数：
        ...
        - **proxy** 字典，默认为空字典，结构：{
                'proxy': '代理服务器的 __URL__(完整 URL)，不允许为空',
                'user': '提供账号，默认为 None',
                'password': '提供密码，默认为 None',
                'not_proxy': [不走代理的地址，每个元素存储一个字符串，默认为 None]
            }
    ...
    1. **waiting_DOM** 等待页面元素出现，参数：
        - **css_selector** CSS选择器，用于查找页面元素
        - **text** 元素的文本内容，为 __None__ 时直接用CSS查找元素，为 __字符串__ 时表示指定元素的文本内容
        - **min_number** 当存在多个满足条件的元素时，需指定至少多少个元素出现才停止，默认 __1__
        - **timeout** 等待超时上限，单位 __秒__，默认 __30__
    ...
    1. **click** 直接点击页面元素，先移动，再点击，参数：
        - **css_selector** __CSS选择器__，用于查找标签
        - **text** 元素的文本内容，为 __None__ 时直接用CSS查找元素，为__字符串__时表示指定元素的文本内容
        - **index** 元素的索引，默认为 __None__，当存在多个被选中元素时，需要提供 __整数__ 参数表示获取指定索引的元素
        - **key** 触发的按键名，`['left', 'middle', 'right']` 分别表示 `['左键', '中键', '右键']`

    1. **get_element_coordinate** 获取页面元素坐标，参数：
        - **css_selector** CSS选择器，用于查找页面元素
        - **text** 元素的文本内容，为 __None__ 时直接用CSS查找元素，为 __字符串__ 时表示指定元素的文本内容
        - **index** 索引，存在多个选中标签时，需提供索引指定第 index 个标签
    ...
    1. **mouse_wheel** 模拟鼠标滚轮，参数：
        - **delta_x** X轴滚动量，__正数__ 时向右滚动指定像素，__负数__ 时向左滚动指定像素
        - **delta_y** Y轴滚动量，__正数__ 时向下滚动指定像素，__负数__ 时向上滚动指定像素

    1. **mouse_down** 单个鼠标按键按下，参数：
        - **button** 按下的按键名，`['left', 'middle', 'right']` 分别表示 `['左键', '中键', '右键']`

    1. **mouse_up** 单个鼠标按键抬起，参数：
        - **button** 抬起的按键名，`['left', 'middle', 'right']` 分别表示 `['左键', '中键', '右键']`

    1. **key_down** 单个键盘按键按下，参数：
        **key** 按下的按键名
